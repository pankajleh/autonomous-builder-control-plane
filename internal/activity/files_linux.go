...
func physicalID(info os.FileInfo) string {
	st, ok := info.Sys().(*syscall.Stat_t)
	if !ok {
		return ""
	}
	return jsonDigest([]uint64{uint64(st.Dev), st.Ino})
}

// statx birth time distinguishes unlink/recreate even when the filesystem
// immediately reuses an inode. ctime cannot do this: normal appends change it.
// Unsupported architectures/filesystems simply make provider detail unknown.
func generationFileID(file *os.File) string {
	var number uintptr
	...
	var data [256]byte
	empty := byte(0)
	_, _, errno := syscall.Syscall6(number, file.Fd(), uintptr(unsafe.Pointer(&empty)), 0x1100, 0xfff, uintptr(unsafe.Pointer(&data[0])), 0)
	if errno != 0 || binary.NativeEndian.Uint32(data[:4])&0x800 == 0 {
		return ""
	}
	info, err := file.Stat()
	if err != nil {
	...
	// Birth timestamps may share a filesystem clock tick across immediate
	// inode reuse. The filesystem inode generation disambiguates that case.
	var generation uint64
	_, _, errno = syscall.Syscall(syscall.SYS_IOCTL, file.Fd(), 0x80087601, uintptr(unsafe.Pointer(&generation)))
	if errno != 0 {
		return ""
	}
	return identity(physicalID(info), hex.EncodeToString(data[80:96]), fmt.Sprint(generation))
}
