...
type cancellationSubmissionProofWireV1 struct {
	...
	TargetSubmission       json.RawMessage                   `json:"target_submission,omitempty"`
	TargetSubmissionSHA256 string                            `json:"target_submission_sha256,omitempty"`
...
func NewCancellationSubmissionProofV1(input CancellationSubmissionProofV1Input, limits Limits) (CancellationSubmissionProofV1, error) {
	...
	hasSubmission := input.TargetSubmission != nil
	...
	if hasSubmission && (!hasAttempt || requireLimitsSHA(limits, input.TargetSubmission.limitsSHA) != nil ||
		input.TargetSubmission.commitmentSHA != input.CommitmentSHA256 ||
		input.TargetSubmission.sealSHA != input.SealSHA256 || input.TargetSubmission.writeID != input.Attempt.WriteID()) {
		return CancellationSubmissionProofV1{}, errors.New("cancellation target submission changed its attempt, seal, or commitment")
	}
	switch input.Kind {
	case CancellationProofNoAdmission:
		if hasAttempt || input.AdmissionSHA256 != "" || input.SealSHA256 != "" || input.CommitmentSHA256 != "" ||
			hasSubmission || input.RequestBytes != 0 || input.SubmissionState != ReconciliationNotApplied || input.NotAppliedProof != nil {
	...
	case CancellationProofSealedZeroRequestBytes:
		if !hasAttempt || !validSHA256(input.AdmissionSHA256) || !validSHA256(input.SealSHA256) || !validSHA256(input.CommitmentSHA256) ||
			!hasSubmission || !input.TargetSubmission.valid() || input.RequestBytes != 0 || input.SubmissionState != ReconciliationNotApplied || input.NotAppliedProof == nil ||
			...
			input.NotAppliedProof.CommitmentSHA256() != input.CommitmentSHA256 || input.NotAppliedProof.submission.SHA256() != input.TargetSubmission.SHA256() ||
			!bytes.Equal(input.NotAppliedProof.submission.CanonicalJSON(), input.TargetSubmission.CanonicalJSON()) ||
	...
	case CancellationProofUnresolvedSubmission:
		if !hasAttempt || !validSHA256(input.AdmissionSHA256) || !validSHA256(input.SealSHA256) || !validSHA256(input.CommitmentSHA256) ||
			!hasSubmission || !input.TargetSubmission.valid() || input.RequestBytes <= 0 || input.SubmissionState != ReconciliationUnknown || input.NotAppliedProof != nil {
	...
	case CancellationProofAuthenticatedNotApplied:
		if !hasAttempt || !validSHA256(input.AdmissionSHA256) || !validSHA256(input.SealSHA256) || !validSHA256(input.CommitmentSHA256) ||
			...
			input.NotAppliedProof.submission.SHA256() != input.TargetSubmission.SHA256() ||
			!bytes.Equal(input.NotAppliedProof.submission.CanonicalJSON(), input.TargetSubmission.CanonicalJSON()) ||
			input.NotAppliedProof.input.RequestBytes != input.RequestBytes || input.NotAppliedProof.input.EvidenceRef != input.EvidenceRef {
...
func cancellationSubmissionProofWire(input CancellationSubmissionProofV1Input, limitsSHA string) cancellationSubmissionProofWireV1 {
	...
	var targetSubmission json.RawMessage
	var targetSubmissionSHA string
	if input.TargetSubmission != nil {
		targetSubmission = input.TargetSubmission.CanonicalJSON()
		targetSubmissionSHA = input.TargetSubmission.SHA256()
	}
	...
	return cancellationSubmissionProofWireV1{
		...
		targetSubmission, targetSubmissionSHA, input.RequestBytes, input.SubmissionState, notApplied, notAppliedSHA, input.EvidenceRef, limitsSHA,
	}
}
...
func ParseCanonicalCancellationSubmissionProofV1(data []byte, limits Limits) (CancellationSubmissionProofV1, error) {
	...
	var targetSubmission *TargetSubmissionV1
	if len(wire.TargetSubmission) > 0 {
		value, err := parseUnboundTargetSubmissionV1(wire.TargetSubmission, limits)
		if err != nil || value.SHA256() != wire.TargetSubmissionSHA256 {
			return CancellationSubmissionProofV1{}, errors.New("cancellation target submission identity disagrees")
		}
		targetSubmission = &value
	} else if wire.TargetSubmissionSHA256 != "" {
		return CancellationSubmissionProofV1{}, errors.New("cancellation target submission digest lacks a record")
	}
	...
	value, err := NewCancellationSubmissionProofV1(CancellationSubmissionProofV1Input{
		...
		CommitmentSHA256: wire.CommitmentSHA256, TargetSubmission: targetSubmission, RequestBytes: wire.RequestBytes, SubmissionState: wire.SubmissionState,
...
func parseUnboundNotAppliedProof(data []byte, limits Limits) (NotAppliedProofV1, error) {
	...
	submission, err := parseUnboundTargetSubmissionV1(wire.TargetSubmission, limits)
...
func cloneCancellationSubmissionProofInput(input CancellationSubmissionProofV1Input) CancellationSubmissionProofV1Input {
	...
	if input.TargetSubmission != nil {
		value := cloneTargetSubmission(*input.TargetSubmission)
		input.TargetSubmission = &value
	}
	return input
}
