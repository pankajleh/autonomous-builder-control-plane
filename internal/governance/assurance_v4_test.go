...
func TestFrozenCatalogPositiveGenerationFailsClosedOnIncompleteSemantics(t *testing.T) {
	...
	if err == nil {
		t.Fatal("full frozen catalog accepted incomplete semantic authority")
	}
	if positives != nil {
		t.Fatal("semantic failure returned partial positive authority")
	}
}

func TestFrozenCatalogRejectionGenerationFailsClosedOnIncompleteSemantics(t *testing.T) {
	extraction, err := ExtractFrozenCanonicalVectorCatalogV1(readFrozenAssuranceModel(t))
	if err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		t.Fatal("full frozen rejection generation accepted incomplete semantic authority")
	}
	if vectors != nil {
		t.Fatal("semantic failure returned partial rejection authority")
	}
}
