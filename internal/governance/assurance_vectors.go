...
type canonicalPredicateContextV1 struct {
	...
	checkpointProjection  map[string]json.RawMessage
}
...
func generateCanonicalCatalogPositiveVectorsV1(catalog CanonicalVectorCatalogV1) (map[string][]byte, *canonicalPredicateContextV1, error) {
	...
	if checkpoint, ok := byRecord["PhaseCheckpointV2"]; ok {
		projection, err := checkpointGrantParentProjectionV1(prototypes[checkpoint.SchemaID])
		if err != nil {
			return nil, nil, err
		}
	...
	if checkpoint, ok := byRecord["PhaseCheckpointV2"]; ok {
		projection, projectionErr := checkpointGrantParentProjectionV1(result[checkpoint.SchemaID])
		if projectionErr != nil || !equalRawMessageMapsV1(context.checkpointProjection, projection) {
			return nil, nil, errors.New("P-PARENT-001 checkpoint projection changed during finalization")
		}
		context.checkpointProjection = projection
	}
	if err := assertFinalTypedReferenceContextV1(catalog, result, context); err != nil {
		return nil, nil, err
	}
	for _, entry := range catalog.Entries {
...
func (context *canonicalPredicateContextV1) addFrozenRetainedRecords() error {
	...
	for recordType, encoded := range records {
		if !json.Valid(encoded) {
			return fmt.Errorf("retained canonical record %s is invalid", recordType)
		}
		context.addRecord(recordType, encoded)
	}
	return nil
}

func frozenRetainedCanonicalRecordsV1() (map[string][]byte, error) {
	activation, err := SealGovernanceActivationV1(GovernanceActivationV1{
		...
		PolicySHA256: strings.Repeat("1", 64), ActivationRepositoryCommit: strings.Repeat("2", 40),
		ActivationSequence: 1, ActivationTime: "1970-01-01T00:00:00Z", GrandfatheredV2Digests: []string{},
	})
	if err != nil {
		return nil, fmt.Errorf("finalize retained GovernanceActivationV1: %w", err)
	}
	...
	bounds := contextcapsule.ExecutionBoundsV1{
		MaxIterations: 1, SessionTimeout: "1s", IdleTimeout: "1s", WallClockTimeout: "1s", AggregateWallClockTimeout: "1s",
		Finalize: false, MaxIncompleteTasks: 1, MaxInitialActiveFindings: 1, MaxRalphexInvocations: 1, MaxReviewReports: 1,
		MaxMutationLeases: 1, MaxTotalFixBatches: 1, MaxChangedFiles: 1, MaxChangedBytes: 1,
	}
	if err := contextcapsule.ValidateExecutionBoundsV1(bounds); err != nil {
		return nil, fmt.Errorf("finalize retained ExecutionBoundsV1: %w", err)
	}
	boundsBytes, err := json.Marshal(bounds)
	if err != nil {
		return nil, err
	}
	checkpoint, err := SealPhaseCheckpointV1(PhaseCheckpointV1{
		Kind: CheckpointDesignAccepted, Repository: "example/project", CapsuleFileSHA256: strings.Repeat("3", 64),
		CapsuleSHA256: strings.Repeat("4", 64), CandidateSHA: strings.Repeat("5", 40), Operation: contextcapsule.OperationDesignReview,
		Verdict: "DESIGN_ACCEPTED", Evidence: []EvidenceBindingV1{{Ref: "evidence/design", SHA256: strings.Repeat("6", 64)}},
		ControllerPolicyIdentity: "policy-v3", ControllerEventIdentity: "event-0", SemanticRegistrySHA256: strings.Repeat("7", 64),
		NextStageGrantSHA256: strings.Repeat("8", 64),
	})
	if err != nil {
		return nil, fmt.Errorf("finalize retained PhaseCheckpointV1: %w", err)
	}
	checkpointBytes, err := json.Marshal(checkpoint)
	if err != nil {
		return nil, err
	}
	// Only bytes emitted by an actual frozen constructor are installed. Other
	// retained types deliberately remain absent, so a catalog that needs one
	// fails closed instead of hashing a schema-shaped placeholder.
	records := map[string][]byte{
		"GovernanceActivationV1": activationBytes,
		"ExecutionBoundsV1":      boundsBytes,
		"PhaseCheckpointV1":      checkpointBytes,
	}
	return records, nil
}
...
func selectedDigestTargetMembersV1(target string, values map[string]json.RawMessage) []string {
	...
	if strings.Contains(target, "GitHubUserObservationV1-or-") {
		var parser string
		_ = json.Unmarshal(values["parser"], &parser)
		selected := map[string]string{
			"GITHUB_USER_V1": "GitHubUserObservationV1", "GITHUB_APP_V1": "GitHubAppObservationV1",
			"GITHUB_INSTALLATION_V1": "GitHubInstallationObservationV1", "GITHUB_REPOSITORY_V1": "GitHubRepositoryObservationV1",
			"GITHUB_INSTALLATION_REPOSITORIES_PAGE_V1": "GitHubInstallationRepositoriesPageV1",
			"GITHUB_PULL_REQUEST_V1":                   "GitHubPullRequestObservationV1",
			"GITHUB_PULL_REQUEST_PAGE_V1":              "GitHubPullRequestPageObservationV1",
			"GITHUB_GIT_REF_V1":                        "GitHubGitRefObservationV1",
			"GITHUB_GIT_COMMIT_V1":                     "GitHubGitCommitObservationV1",
		}[parser]
		if selected != "" && containsCanonicalVectorStringV1(members, selected) {
...
func (context *canonicalPredicateContextV1) digestValidForTarget(target, digest string) bool {
	...
	for _, member := range digestTargetMembersV1(target) {
		if _, ok := context.recordByTypeAndDigest[member][digest]; ok {
			return true
		}
	}
	return false
...
func assertFinalTypedReferenceContextV1(catalog CanonicalVectorCatalogV1, finals map[string][]byte, context *canonicalPredicateContextV1) error {
	for _, entry := range catalog.Entries {
		...
		for index, field := range entry.Fields {
			...
			for _, candidate := range candidates {
				var digest string
				if json.Unmarshal(candidate, &digest) != nil || !context.digestValidForSelectedTarget(field.DigestTarget, digest, values) {
				...
				for _, member := range selectedDigestTargetMembersV1(field.DigestTarget, values) {
					...
					if len(final) != 0 && digest == sha256Hex(final) {
						matchedFinal = true
						break
					}
				}
				if !matchedFinal {
					return fmt.Errorf("%s.%s digest does not equal selected final target bytes", entry.SchemaID, field.FieldPath)
				}
			}
		}
	}
	...
	case PredicateTypedReference:
		...
		for _, path := range paths {
			...
			for _, candidate := range candidates {
				...
				if context.digestValidForSelectedTarget(field.DigestTarget, digest, values) {
					continue
				}
				return false
			}
...
func frozenCanonicalPredicateValidV1(entry CanonicalVectorEntryV1, predicate PredicateDescriptorV1, paths []string, fields map[string]WireFieldDescriptorV1, values map[string]json.RawMessage, context *canonicalPredicateContextV1) (bool, bool) {
	...
	if _, compatible := frozenCrossRecordPredicateIDsV1[predicate.PredicateID]; compatible {
		return frozenCrossRecordSemanticValidV1(entry, predicate, values, context), true
	}
	return false, false
}
...
const (
	...
	frozenCrossStageSubjectV1  frozenCrossRecordSemanticFamilyV1 = "STAGE_SUBJECT_RELATION"
	frozenCrossCheckpointV1    frozenCrossRecordSemanticFamilyV1 = "CHECKPOINT_PARENT_PROJECTION"
	frozenCrossStageLineageV1  frozenCrossRecordSemanticFamilyV1 = "STAGE_LINEAGE_AUTHORITY"
...
// frozenCrossRecordSemanticRegistryV1 is the closed executable dispatch table.
// Every registered frozen cross-record predicate has one record-specific
// evaluator family; absence is a fail-closed model error.
var frozenCrossRecordSemanticRegistryV1 = map[string]frozenCrossRecordSemanticFamilyV1{
	"ArtifactPublisherAbandonV1\x00P-PUBLISHER-007":                                              frozenCrossFailClosedV1,
	"ArtifactPublisherAppendV1\x00P-PUBLISHER-004":                                               frozenCrossFailClosedV1,
	"ArtifactPublisherFinalizeV1\x00P-PUBLISHER-005":                                             frozenCrossFailClosedV1,
	"ArtifactPublisherOpenV1\x00P-PUBLISHER-003":                                                 frozenCrossFailClosedV1,
	"AssuranceResourceReservationV1\x00P-RESOURCE-003":                                           frozenCrossFailClosedV1,
	"AuthenticatedProviderPrincipalV1\x00P-EFFECT-001":                                           frozenCrossFailClosedV1,
	"CandidateDiffArtifactV1\x00P-DIFF-003":                                                      frozenCrossDiffArtifactV1,
	"ContainerAbsenceObservationV1\x00P-container-absence-observation-v1-ROW-002":                frozenCrossFailClosedV1,
	"EffectAttemptIndexV1\x00P-effect-attempt-index-v1-ROW-001":                                  frozenCrossFailClosedV1,
	"EffectAttemptIndexV1\x00P-effect-attempt-index-v1-ROW-002":                                  frozenCrossFailClosedV1,
	"EffectBarrierResolutionV1\x00P-BARRIER-007":                                                 frozenCrossFailClosedV1,
	"EffectCallAdmissionV1\x00P-BARRIER-004":                                                     frozenCrossFailClosedV1,
	"EffectLedgerTransitionAuthorityV1\x00P-BARRIER-003":                                         frozenCrossFailClosedV1,
	"EffectReauthorizationV1\x00P-effect-reauthorization-v1-ROW-001":                             frozenCrossFailClosedV1,
	"EffectReconciliationAnchorV1\x00P-RECONCILE-001":                                            frozenCrossFailClosedV1,
	"EffectTransitionBarrierV1\x00P-BARRIER-002":                                                 frozenCrossFailClosedV1,
	"EffectUnclaimedAbandonmentV1\x00P-BARRIER-005":                                              frozenCrossFailClosedV1,
	"ExecutionIdentityV1\x00P-RESOURCE-003":                                                      frozenCrossFailClosedV1,
	"GitHubGitRefObservationV1\x00P-github-git-ref-observation-v1-ROW-001":                       frozenCrossFailClosedV1,
	"GitHubInstallationRepositoriesPageV1\x00P-github-installation-repositories-page-v1-ROW-001": frozenCrossFailClosedV1,
	"ControllerStateV2\x00P-governance-controller-state-v2-ROW-002":                              frozenCrossFailClosedV1,
	"LedgerEvidenceRefListV1\x00P-ledger-evidence-ref-list-v1-ROW-001":                           frozenCrossFailClosedV1,
	"LedgerEvidenceRefListV1\x00P-ledger-evidence-ref-list-v1-ROW-002":                           frozenCrossFailClosedV1,
	"MergeEffectRequestV1\x00P-MERGE-001":                                                        frozenCrossFailClosedV1,
	"MergeEffectRequestV1\x00P-MERGE-002":                                                        frozenCrossFailClosedV1,
	"MergeProviderObservationV1\x00P-merge-provider-observation-v1-ROW-002":                      frozenCrossFailClosedV1,
	"MergeReadinessAuthorityV1\x00P-READY-001":                                                   frozenCrossFailClosedV1,
	"CheckpointGrantParentV2\x00P-PARENT-001":                                                    frozenCrossCheckpointV1,
	"PhaseAuthorityV4\x00P-phase-authority-v4-ROW-001":                                           frozenCrossFailClosedV1,
	"PhaseCheckpointV2\x00P-phase-checkpoint-v2-ROW-002":                                         frozenCrossFailClosedV1,
	"PREffectReconciliationV1\x00P-RECONCILE-002":                                                frozenCrossFailClosedV1,
	"PREffectReconciliationV1\x00P-pr-effect-reconciliation-v1-ROW-003":                          frozenCrossFailClosedV1,
	"PREffectRequestV1\x00P-PR-001":                                                              frozenCrossFailClosedV1,
	"PREffectRequestV1\x00P-pr-effect-request-v1-ROW-004":                                        frozenCrossFailClosedV1,
	"PRProviderObservationV1\x00P-PR-003":                                                        frozenCrossFailClosedV1,
	"PRProviderObservationV1\x00P-pr-provider-observation-v1-ROW-004":                            frozenCrossFailClosedV1,
	"PRPublicationAuthorityV1\x00P-pr-publication-authority-v1-ROW-001":                          frozenCrossFailClosedV1,
	"ProcessAbsenceObservationV1\x00P-process-absence-observation-v1-ROW-001":                    frozenCrossProcessProbeV1,
	"ProcessAbsenceObservationV1\x00P-process-absence-observation-v1-ROW-002":                    frozenCrossFailClosedV1,
	"ProviderAuthenticationContextV1\x00P-EFFECT-001":                                            frozenCrossFailClosedV1,
	"ProviderAuthenticationContextV1\x00P-provider-authentication-context-v1-ROW-002":            frozenCrossFailClosedV1,
	"ProviderReadRequestV1\x00P-EFFECT-002":                                                      frozenCrossFailClosedV1,
	"ProviderReadResponseV1\x00P-provider-read-response-v1-ROW-001":                              frozenCrossFailClosedV1,
	"ProviderReadResponseV1\x00P-provider-read-response-v1-ROW-009":                              frozenCrossFailClosedV1,
	"ProviderRequestV1\x00P-EFFECT-002":                                                          frozenCrossFailClosedV1,
	"PublisherAbandonAuthorizationV1\x00P-PUBLISHER-006":                                         frozenCrossPublisherTimeV1,
	"PublisherRecoveryProofV1\x00P-PUBLISHER-006":                                                frozenCrossFailClosedV1,
	"ReadyForMergeEvidenceV1\x00P-READY-001":                                                     frozenCrossFailClosedV1,
	"SecretScanAuthorityV1\x00P-DIFF-004":                                                        frozenCrossFailClosedV1,
	"SecretScanEvidenceV2\x00P-DIFF-005":                                                         frozenCrossFailClosedV1,
	"StageDiffLineageV1\x00P-DIFF-002":                                                           frozenCrossStageLineageV1,
	"StageDiffLineageV1\x00P-stage-diff-lineage-v1-ROW-003":                                      frozenCrossFailClosedV1,
	"StageSubjectV1\x00P-SUBJECT-002":                                                            frozenCrossStageSubjectV1,
...
func frozenCrossRecordSemanticValidV1(entry CanonicalVectorEntryV1, predicate PredicateDescriptorV1, values map[string]json.RawMessage, context *canonicalPredicateContextV1) bool {
	family, explicit := frozenCrossRecordSemanticRegistryV1[frozenCrossRecordSemanticKeyV1(entry.RecordName, predicate.PredicateID)]
	if !explicit || context == nil {
		return false
	}
	switch family {
	case frozenCrossDiffArtifactV1:
		return frozenCandidateDiffRelationValidV1(values, context)
	case frozenCrossStageSubjectV1:
		return frozenStageSubjectRelationValidV1(values, context)
	case frozenCrossCheckpointV1:
		return frozenCheckpointParentProjectionValidV1(values, context)
	case frozenCrossStageLineageV1:
		return frozenStageDiffAuthorityValidV1(values, context)
	case frozenCrossProcessProbeV1:
		var pid uint64
		var path string
		return json.Unmarshal(values["expected_pid"], &pid) == nil && json.Unmarshal(values["proc_stat_path"], &path) == nil && path == "/proc/"+strconv.FormatUint(pid, 10)+"/stat"
	case frozenCrossWorkerKeyV1:
		return frozenWorkerObserverIdentityValidV1(values)
	case frozenCrossPublisherTimeV1:
		return frozenStatePredicateV1(entry, predicate, nil, values)
	case frozenCrossFailClosedV1:
		return false
	default:
		return false
	}
...
func frozenCandidateDiffRelationValidV1(values map[string]json.RawMessage, context *canonicalPredicateContextV1) bool {
	subject, ok := referencedRecordValuesV1(values, "stage_subject_sha256", "StageSubjectV1", context)
	if !ok {
		return false
	}
	lineage, ok := referencedRecordValuesV1(values, "diff_lineage_sha256", "StageDiffLineageV1", context)
	if !ok {
		return false
	}
	return equalRawFieldsV1(values, subject, "repository_identity", "repository_identity") &&
		equalRawFieldsV1(values, lineage, "repository_identity", "repository_identity") &&
		equalRawFieldsV1(values, subject, "accepted_a_lineage_sha256", "accepted_a_lineage_sha256") &&
		equalRawFieldsV1(values, lineage, "accepted_a_lineage_sha256", "accepted_a_lineage_sha256") &&
		equalRawFieldsV1(values, subject, "diff_lineage_sha256", "diff_lineage_sha256") &&
		equalRawFieldsV1(values, subject, "base_oid", "diff_base_oid") &&
		equalRawFieldsV1(values, lineage, "base_oid", "base_oid") &&
		equalRawFieldsV1(values, subject, "candidate_oid", "diff_candidate_oid") &&
		equalRawFieldsV1(values, lineage, "candidate_oid", "candidate_oid") &&
		equalRawFieldsV1(subject, lineage, "repository_identity", "repository_identity") &&
		equalRawFieldsV1(subject, lineage, "accepted_a_lineage_sha256", "accepted_a_lineage_sha256") &&
		equalRawFieldsV1(subject, lineage, "diff_base_oid", "base_oid") &&
		equalRawFieldsV1(subject, lineage, "diff_candidate_oid", "candidate_oid")
}

func frozenStageSubjectRelationValidV1(values map[string]json.RawMessage, context *canonicalPredicateContextV1) bool {
	lineage, ok := referencedRecordValuesV1(values, "diff_lineage_sha256", "StageDiffLineageV1", context)
	return ok && equalRawFieldsV1(values, lineage, "repository_identity", "repository_identity") &&
		equalRawFieldsV1(values, lineage, "accepted_a_lineage_sha256", "accepted_a_lineage_sha256") &&
		equalRawFieldsV1(values, lineage, "stage", "stage") &&
		equalRawFieldsV1(values, lineage, "diff_base_oid", "base_oid") &&
		equalRawFieldsV1(values, lineage, "diff_candidate_oid", "candidate_oid")
}

func referencedRecordValuesV1(values map[string]json.RawMessage, digestPath, recordType string, context *canonicalPredicateContextV1) (map[string]json.RawMessage, bool) {
	var digest string
	if context == nil || json.Unmarshal(values[digestPath], &digest) != nil {
		return nil, false
	}
	record, ok := context.recordForDigest(recordType, digest)
	if !ok {
		return nil, false
	}
	decoded, _, duplicate, err := decodeCanonicalVectorObjectV1(record)
	return decoded, err == nil && !duplicate
}

func equalRawFieldsV1(left, right map[string]json.RawMessage, leftPath, rightPath string) bool {
	leftValue, leftPresent := left[leftPath]
	rightValue, rightPresent := right[rightPath]
	return leftPresent && rightPresent && bytes.Equal(leftValue, rightValue)
}

func frozenWorkerObserverIdentityValidV1(values map[string]json.RawMessage) bool {
	readString := func(path string) (string, bool) {
		var value string
		err := json.Unmarshal(values[path], &value)
		return value, err == nil
	}
	worker, workerOK := readString("worker_identity")
	host, hostOK := readString("host_identity")
	keyID, keyOK := readString("key_id")
	observer, observerOK := readString("observer_identity")
	if !workerOK || !hostOK || !keyOK || !observerOK {
		return false
	}
	digest := sha256.Sum256([]byte("ABCP-WORKER-OBSERVER-V1\x00" + worker + "\x00" + host + "\x00" + keyID))
	return observer == hex.EncodeToString(digest[:])
}
...
func frozenCheckpointParentProjectionValidV1(values map[string]json.RawMessage, context *canonicalPredicateContextV1) bool {
	if context == nil || len(context.checkpointProjection) == 0 {
		return false
	}
	return equalRawMessageMapsV1(values, context.checkpointProjection)
}

func applyFrozenCrossRecordPositiveV1(entry CanonicalVectorEntryV1, predicate PredicateDescriptorV1, values map[string]json.RawMessage, context *canonicalPredicateContextV1) error {
	switch predicate.PredicateID {
	case "P-PARENT-001":
		if len(context.checkpointProjection) == 0 {
			return errors.New("P-PARENT-001 has no exact finalized checkpoint")
		}
		for path := range values {
			value, present := context.checkpointProjection[path]
			if !present {
				delete(values, path)
				continue
			}
		...
		for path, value := range context.checkpointProjection {
	...
	if family, explicit := frozenCrossRecordSemanticRegistryV1[frozenCrossRecordSemanticKeyV1(entry.RecordName, predicate.PredicateID)]; explicit {
		switch family {
		case frozenCrossDiffArtifactV1:
			subject, ok := referencedRecordValuesV1(values, "stage_subject_sha256", "StageSubjectV1", context)
			if !ok {
				return errors.New("P-DIFF-003 subject is unavailable")
			}
			for destination, source := range map[string]string{
				"repository_identity": "repository_identity", "accepted_a_lineage_sha256": "accepted_a_lineage_sha256",
				"base_oid": "diff_base_oid", "candidate_oid": "diff_candidate_oid",
			} {
				values[destination] = append(json.RawMessage(nil), subject[source]...)
			}
		case frozenCrossStageSubjectV1:
			lineage, ok := referencedRecordValuesV1(values, "diff_lineage_sha256", "StageDiffLineageV1", context)
			if !ok {
				return errors.New("P-SUBJECT-002 lineage is unavailable")
			}
			for destination, source := range map[string]string{
				"repository_identity": "repository_identity", "accepted_a_lineage_sha256": "accepted_a_lineage_sha256", "stage": "stage",
				"diff_base_oid": "base_oid", "diff_candidate_oid": "candidate_oid",
			} {
				values[destination] = append(json.RawMessage(nil), lineage[source]...)
			}
		case frozenCrossProcessProbeV1:
			var pid uint64
			if json.Unmarshal(values["expected_pid"], &pid) != nil {
				return errors.New("process probe PID is invalid")
			}
			values["proc_stat_path"] = mustMarshalCanonicalVectorStringV1("/proc/" + strconv.FormatUint(pid, 10) + "/stat")
		case frozenCrossWorkerKeyV1:
			...
			digest := sha256.Sum256([]byte("ABCP-WORKER-OBSERVER-V1\x00" + read("worker_identity") + "\x00" + read("host_identity") + "\x00" + read("key_id")))
			values["observer_identity"] = mustMarshalCanonicalVectorStringV1(hex.EncodeToString(digest[:]))
		case frozenCrossFailClosedV1:
			return fmt.Errorf("%s for %s has no complete literal frozen evaluator", predicate.PredicateID, entry.RecordName)
		}
	}
	return nil
}

func checkpointGrantParentProjectionV1(checkpointBytes []byte) (map[string]json.RawMessage, error) {
	values, _, duplicate, err := decodeCanonicalVectorObjectV1(checkpointBytes)
	if err != nil || duplicate {
		return nil, errors.New("P-PARENT-001 source checkpoint is not canonical")
	}
	delete(values, "next_stage_grant_sha256")
	delete(values, "checkpoint_sha256")
	if len(values) == 0 {
		return nil, errors.New("P-PARENT-001 source checkpoint projection is empty")
	}
	return values, nil
}

func equalRawMessageMapsV1(left, right map[string]json.RawMessage) bool {
	if len(left) != len(right) {
		return false
	}
	for path, value := range left {
		other, present := right[path]
...
var frozenCrossRecordPredicateIDsV1 = map[string]struct{}{
	...
	"P-pr-effect-reconciliation-v1-ROW-003":        {},
...
func mutatePredicateV1(entry CanonicalVectorEntryV1, predicate PredicateDescriptorV1, values map[string]json.RawMessage, recordPositives map[string][]byte, context *canonicalPredicateContextV1) error {
	...
	if _, compatible := frozenCrossRecordPredicateIDsV1[predicate.PredicateID]; compatible {
		return mutateFrozenCrossRecordSemanticV1(entry, predicate, values, context)
	}
	switch predicate.Operator {
	case PredicateExactLiteral:
