...
The governed authority plan and its admitted SHA-256 remain immutable controller evidence. When worktree execution uses a product-admission plan that is Git-ignored, the controller writes the same bytes to a restricted, hash-verified `abcp-ralphex-plan-<run-id-digest>/plan.md` copy, passes that copy to Ralphex, and removes it after the process returns or setup fails. A repository-scoped execution lease serializes this Git-visible handoff with every other governed run against the same Git common directory. Durable controller-owned metadata binds each copy to its run-derived path and admitted SHA-256; after an interrupted process releases the operating-system lease, the next holder removes only an exactly matching stale copy before clean-tree validation. The reserved `abcp-ralphex-plan-*` namespace must remain visible to Git; admission profiles whose repository ignore rules hide it are rejected. Tracked authority plans continue to use their governed path directly only when the plan blob in the governed start commit has the admitted SHA-256; hidden index flags cannot substitute different source-checkout bytes. Failure to verify a tracked blob or to create, verify, expose, recover, or remove a required copy fails the run.
