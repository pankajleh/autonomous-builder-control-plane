...
The controller-owned profile maps only `profile_id` to private repository/execution configuration. It must be a protected regular file and binds repository path/identity, manifest template, ignored materialization directory, ledger/evidence/cgroup roots, and a protected workflow-authority configuration path. Repository ignore rules must leave the reserved `abcp-ralphex-plan-*` handoff namespace visible to Git; broad ignore rules require explicit negations for that namespace.

Admission deterministically materializes the product plan, V2 context capsule, and private authority manifest, then starts the existing `abcp run` command. The plan contains exactly one incomplete executable `### Task 1` wrapper and one actionable unchecked item. The complete authorized `task_markdown` remains byte-for-byte intact inside a collision-safe backtick or tilde fence, so headings and checkboxes in the authorized payload cannot become additional executable tasks. Task markdown remains bounded to 64 KiB overall and to 65,535 bytes per line so the pinned Ralphex plan parser can read every admitted plan. A per-run launch lock suppresses duplicate launch; a registered catalog run is never relaunched. A separate repository-scoped execution lease serializes Git-visible Ralphex handoffs across distinct admitted runs that target the same checkout.
