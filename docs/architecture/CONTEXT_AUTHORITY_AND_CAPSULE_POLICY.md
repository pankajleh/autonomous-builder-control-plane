...
For `INITIAL_IMPLEMENTATION`, report zero establishes a bounded active-finding set. Later reports may retain or remove IDs but never add one. For `CORRECTION`, every blocker is already named by B and the same monotonic rule applies. An unmapped or newly blocking implementation concern returns `SCOPE_EXPANSION_REQUIRED`; a `DESIGN_GAP` or `ASSURANCE_MODEL_GAP` returns to a new A lineage. Final-review discovery of a missing material failure class is therefore not correction authority.
