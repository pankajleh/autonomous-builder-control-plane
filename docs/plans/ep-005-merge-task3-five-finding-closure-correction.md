...
- the invocation policy must make impossible class reassignment: authorization reads may reserve only `pre-submit`; commit creation only `commit-submission`; target GraphQL mutation only `target` followed by its single verification read charged to `post-merge`; target reconciliation only `reconciliation`; ordinary post-merge observation only `post-merge`; no provider route may consume either of the two reserved validation calls;
